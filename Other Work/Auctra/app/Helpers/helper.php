...
// if (!function_exists('toggleInteraction')) {
//     function toggleInteraction(string $model, array $where, array $extra = [])
//     {
...
//         $existing ? $existing->delete() : $model::create(array_merge($where, $extra, [
//             'user_id' => auth()->id()
//         ]));
//         return true;
//     }
    ...
    function toggleInteraction(string $model, array $where, string $interaction, array $extra = [])
    {
        ...
        foreach ($where as $key => $value) {
            if (str_contains($key, '_type')) {
                ...
                $targetModel = $modelClass::find($where[$idKey]);
                break;
            }
        }
        // ? check if interaction exists & delete it or create a new one with increment | decrement
        $existing ? tap($existing)->delete() && event(new InteractionToggled($targetModel, 'decrement', $interaction))
            : $model::create(array_merge($where, $extra, ['user_id' => auth()->id()])) && event(new InteractionToggled($targetModel, 'increment', $interaction));
        return true;
    }
}
